...
class Highway(tf.keras.layers.Layer):
    ...
    # Arguments
        ...
        kernel_constraint: Constraint function applied to
            the `kernel` weights matrix
            (see [constraints](../tf.keras.constraints.md)).
    ...
    def __init__(self,
        ...
        self.transform_activation = tf.keras.activations.get(transform_activation)

        self.kernel_initializer = tf.keras.initializers.get(kernel_initializer)
        self.transform_initializer = tf.keras.initializers.get(transform_initializer)
        self.bias_initializer = tf.keras.initializers.get(bias_initializer)
        if isinstance(transform_bias_initializer, int):
            self.transform_bias_initializer = tf.keras.initializers.Constant(value=transform_bias_initializer)
        else:
            self.transform_bias_initializer = tf.keras.initializers.get(transform_bias_initializer)

        self.kernel_regularizer = tf.keras.regularizers.get(kernel_regularizer)
        self.transform_regularizer = tf.keras.regularizers.get(transform_regularizer)
        self.bias_regularizer = tf.keras.regularizers.get(bias_regularizer)
        self.transform_bias_regularizer = tf.keras.regularizers.get(transform_bias_regularizer)
    ...
    def call(self, x, mask=None):
        x_h = self.activation(tf.keras.backend.dot(x, self.W) + self.bias)
        x_trans = self.transform_activation(tf.keras.backend.dot(x, self.W_transform) + self.bias_transform)
        output = x_h * x_trans + (1 - x_trans) * x
        return output

    def get_config(self):
        config = {'activation': tf.keras.activations.serialize(self.activation),
                  'transform_activation': tf.keras.activations.serialize(self.transform_activation),
                  'kernel_initializer': tf.keras.initializers.serialize(self.kernel_initializer),
                  'transform_initializer': tf.keras.initializers.serialize(self.transform_initializer),
                  'bias_initializer': tf.keras.initializers.serialize(self.bias_initializer),
                  'transform_bias_initializer': tf.keras.initializers.serialize(self.transform_bias_initializer),
                  'kernel_regularizer': tf.keras.regularizers.serialize(self.kernel_regularizer),
                  'transform_regularizer': tf.keras.regularizers.serialize(self.transform_regularizer),
                  'bias_regularizer': tf.keras.regularizers.serialize(self.bias_regularizer),
                  'transform_bias_regularizer': tf.keras.regularizers.serialize(self.transform_bias_regularizer),

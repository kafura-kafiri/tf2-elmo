...
class ELMo(object):
    ...
    def compile_elmo(self, print_summary=False):
        ...
        for i in range(self.parameters['n_lstm_layers']):
            if self.parameters['cuDNN']:
                re_lstm = LSTM(units=self.parameters['lstm_units_size'], return_sequences=True,
                                    kernel_constraint=MinMaxNorm(-1*self.parameters['cell_clip'],
                                                                 self.parameters['cell_clip']),
                                    recurrent_constraint=MinMaxNorm(-1*self.parameters['cell_clip'],
        ...
        self._model.compile(optimizer=tf.keras.optimizers.Adagrad(lr=self.parameters['lr'], clipvalue=self.parameters['clip_value']),
                            loss=None)
        if print_summary:
            self._model.summary()
    ...
    def load(self):
        self._model = tf.keras.models.load_model(os.path.join(MODELS_DIR, 'ELMo_LM.h5'),
                                 custom_objects={'TimestepDropout': TimestepDropout,
                                                 'Camouflage': Camouflage})

    def load_elmo_encoder(self):
        self._elmo_model = tf.keras.models.load_model(os.path.join(MODELS_DIR, 'ELMo_Encoder.hd5'),
                                      custom_objects={'TimestepDropout': TimestepDropout,
                                                      'Camouflage': Camouflage})
    ...
    def reverse(inputs, axes=1):
        return tf.keras.backend.reverse(inputs, axes=axes)
    ...
    def perplexity(y_pred, y_true):
        ...
        for y_pred_seq, y_true_seq in zip(y_pred, y_true):
            ...
            cross_entropy = tf.keras.backend.categorical_crossentropy(tf.keras.backend.tf.convert_to_tensor(y_true_seq, dtype=tf.keras.backend.tf.float32),
                                                       tf.keras.backend.tf.convert_to_tensor(y_pred_seq, dtype=tf.keras.backend.tf.float32))
            cross_entropies.extend(cross_entropy.eval(session=tf.keras.backend.get_session()))
